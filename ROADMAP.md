...
## 8. Proposed Additions (2026-06-18 review)

> A fresh, code-grounded triage pass over the v1.0.0 source. Purely **additive** — no existing section is rewritten, so each item can be accepted, rescheduled into a phase, or declined independently. Bar for inclusion: verified against the current source, genuinely user-facing, and **not already covered** by Phases 6–11 or the §5 non-goals. Items already planned (bulk multi-select 6.4, preset/strategy comparison 8.5, the Phase 6 correctness backlog) were deliberately left out to avoid restating the plan.

The throughline: the scenario feature (Phase 4) shipped **create / activate / delete** but not the rest of an entity's normal lifecycle, and the persistence toggle (Phase 1) clears data with no guard. These are **completeness gaps in already-shipped features**, distinct from the new-capability feature phases — and none falls foul of the non-goals (no backend, no budgeting, stays on-device).

### 8.A — UX safety / data integrity

| #   | Proposed item                                                                                                                                                                                                                                                                          | Rationale                                                                                                                                                                                                                              |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8.1 | **Confirm before disabling "Save on this device."** `usePersistence.toggle` calls `clearData()` immediately on disable and only then shows a _success_ snackbar (`use-persistence.ts:122–129`) — no modal guard, no undo, unlike row deletes that route through `ConfirmDeleteDialog`. | One accidental click silently destroys all on-device data. A confirm dialog (or a soft-undo snackbar that restores) brings the toggle to the same safety bar as a single-row delete — pairs with the Phase 6.3 import-merge undo work. |

### 8.B — Scenario lifecycle (Phase 4 follow-ups)

| #   | Proposed item                                                                                                                                                                                                                                                      | Rationale                                                                                                                                                                                                    |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 8.2 | **Edit an existing scenario.** Scenario chips expose only activate (`onClick`) and delete (`onDelete`); there is no edit affordance (`scenario-bar.tsx:45–57`), even though `ScenarioBuilderDialog` already accepts an optional `scenario` prop it is never given. | Tweaking one extra-payment amount today means deleting and re-entering the whole allocation. Wiring the existing dialog into edit mode is low-cost and completes the CRUD the builder was already built for. |
| 8.3 | **Clone a scenario.** Loans and investments have one-click clone (Phase 3.3); scenarios do not (`scenario-bar.tsx`).                                                                                                                                               | "Compare this plan with one small change" is the core scenario workflow; cloning removes the manual re-entry and mirrors an affordance users already have on positions.                                      |

### 8.C — New features / completeness

| #   | Proposed item                                                                                                                                                                           | Rationale                                                                                                                                                                                        |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 8.4 | **Free-text notes on loans/investments.** The models are entirely numeric/date/enum with no `Notes`/`Tags` field (`loan-model.ts`, `investment-model.ts`).                              | A place to record "0% promo ends 2027" or "house down-payment fund" makes a multi-position portfolio legible. An input-only string field needs no math and rides the existing D8 schema ladder.  |
| 8.5 | **Archive / inactive status for positions.** No model carries an active/archived flag; a paid-off loan or dormant account can only be deleted (`loan-model.ts`, `investment-model.ts`). | Lets users keep a paid-off loan for history without cluttering the tables or (optionally) the forecast — a softer, reversible alternative to delete. Pairs with the Phase 6.4 table filter work. |

**Also noted, folded rather than added:** side-by-side comparison of two _saved_ scenarios (only one overlay is active at a time, `scenario-bar.tsx:53`). Phase 8.5 ("Strategy comparison view") is its natural home — recommend widening that item's scope to cover saved scenarios, not only presets, rather than opening a separate line.
