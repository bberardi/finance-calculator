...
| #    | Work item                                                                  | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ---- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.1 | **Single shared validation schema (import ↔ form parity by construction)** | The JSON import boundary (`data-helpers.ts`) and the per-type edit-form validators (`validation-helpers.ts`) enforce the same rules in **two hand-synced places** — the form code is littered with "mirrors the JSON import boundary" comments — and they keep drifting apart. A recurring class of open bugs is exactly this drift: import accepts a value the edit form then rejects, so the round-tripped position is **uneditable** (a `null` optional housing field #201, a `Principal`/`CurrentAmount` of 0 #194, a negative `GrowthRate` #184), or the form accepts what import rejects (`+Infinity` #183), or import lacks a guard the form has and throws a raw `TypeError` instead of the friendly validation error (#178, #202, #175). Define **one schema/parser per model that both boundaries call**, so a rule cannot exist on only one side. Retires the whole uneditable-round-trip class by construction rather than one bug at a time. No schema-version bump; upholds D7 (pure, framework-free) and routes through the D8 migration ladder unchanged. Math-free, so the Charter's reference tests don't apply, but the shared parser gets its own boundary test matrix. |

**Dashboard insight (UX / visualization)**

| #    | Work item                      | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ---- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.2 | **Key financial ratios strip** | The dashboard is four **scalar** cards (`summary-helpers.ts` → total debt / assets / net worth / monthly commitments); the planned 14.3 adds a **composition** view (dollar _amounts_ by category). Neither shows the **relationships** that answer "where should my next dollar go?" most directly: a debt-to-asset ratio, a liquid-vs-illiquid split, a cash-runway figure (months of monthly commitments covered by cash), and — most on-point — the balance-weighted average loan rate **beside** the balance-weighted average expected investment return, which is the debt-vs-invest decision in a single line. Purely derived from the positions, no engine or schema change. Distinct from 14.3 (amounts) and the optimizer (per-plan scoring). |

**Honesty & trust (extends Phase 9 / Phase 15)**

| #    | Work item                          | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.3 | **Live, linked assumptions panel** | The assumptions panel states its assumptions as **static copy** — `assumptions-panel.tsx` hardcodes "discount at 3%/yr" and "an after-tax view is still planned." Several of those assumptions are becoming user-adjustable (15.1 after-tax toggle, 17.5 user-set inflation rate), at which point static copy will contradict the live setting — the exact in-app-copy-vs-plan mismatch the roadmap already cites to justify 15.1 and 16.2. Make each panel line read the user's **actual current setting** (inflation %, after-tax on/off, Monte Carlo on/off) and link it to the control that changes it, so the honesty framing stays honest and becomes a navigation aid. Small, derived, no engine change; best sequenced after 15.1/17.5 land so it reflects real controls. |
