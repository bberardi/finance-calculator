...
### Phase 19 — Privacy Hardening, Chart Reading & Decision Completeness — _target v2.8_

Surfaced by an August 2026 follow-up UX/codebase review. Where Phases 16–18
deepened the _forecast_ and its _what-ifs_, this phase closes gaps the earlier
reviews left in three places the founding pitch actually lives: the **privacy**
promise ("your numbers never leave your device") is enforced only in transit,
not at rest; the **chart** can't be read across a full compounding horizon; and
the **"where does my next dollar go?"** decision model has no concept of a cash
safety buffer. Each item passes the §5 non-goal filters (client-side, data stays
on device, not a budgeting app); the rationale column says why each earns a slot.

**Privacy & data safety**

| #    | Work item                                                         | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.1 | **Passphrase-encrypted export (and optional at-rest encryption)** | The privacy story is the product's headline — the README promises "your numbers never leave your device" — but the data is stored and shared **in plaintext**: `saveData` writes raw JSON to `localStorage` (`storage-helpers.ts:54`) and export produces an unencrypted `.json` file. A backup on a synced/shared drive, or `localStorage` on a shared computer, is readable by anyone with access. Add optional WebCrypto (AES-GCM with a PBKDF2-derived key) passphrase encryption on export — and optionally on the persisted blob — so the off-device copy and the at-rest data are protected with no backend and no stored key. Extends D4/D5; import detects and decrypts the encrypted envelope through the same validation/migration ladder. |
| 19.2 | **"Clear all data now" control + optional idle auto-lock**        | The only way to wipe data today is toggling persistence **off** (`persistence-toggle.tsx` → `clearData`), which is indirect and easy to miss; there is no explicit "erase everything on this device now" action and no inactivity timeout. Add a dedicated, confirmed (soft-undo) _clear-all_ control and an opt-in "auto-clear after N minutes idle" for shared/public machines. Complements the 13.1 unsaved-changes guard and the 18.4 backup reminder, and pairs naturally with 19.1. Purely client-side, one boolean/timeout of new state.                                                                                                                                                                                                       |

**Chart reading (extends Phase 12 / Phase 17)**

| #    | Work item                               | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.3 | **Optional logarithmic net-worth axis** | The forecast chart's y-axis is linear (`forecast-chart.tsx:350`), so on a 30-year compounding curve the early years are visually crushed and differences in _growth rate_ between positions stay unreadable until the last third of the horizon. An opt-in log scale renders constant-growth lines as straight lines and makes rate comparisons legible across the whole range — a standard long-horizon finance-chart affordance. Display-only, no engine change; complements the planned 17.6 zoom/pan and the 15.3 perceptibility work, and keeps the accessible "view as table" fallback for exact values. |

**Decision completeness (extends the founding "next dollar" question)**

| #    | Work item                                                | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.4 | **Emergency-fund / cash-runway target in the optimizer** | The "where does my next dollar go?" ranking scores debt-vs-invest but has **no notion of a cash safety buffer**, which conventional guidance funds _before_ extra debt paydown or investing. Add a single user-set "months of essential expenses" target — **one scalar input, not expense tracking or a budget**, staying inside the §5 line exactly as the employer-match % and the 15.1 after-tax rate are single inputs — that flags when liquid cash sits below target and lets the optimizer prioritize topping it up first. Closes a real hole in the decision model and directly serves the founding question; the single-scalar framing is the same in-bounds reasoning §5 applied to the after-tax haircut. |

**Technical / quality (extends the §4 "executable proof" ethos)**

| #    | Work item                                                     | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.5 | **Automated accessibility regression tests (axe-core in CI)** | The Math Correctness Charter (§4) makes math correctness _executable_ — tests, not review confidence — but the accessibility investments of Phases 6/12/15 have **no equivalent automated guardrail**, so a dropped label or a contrast regression can merge silently. Add `axe-core` assertions to the existing Vitest/Playwright suites over the key surfaces (dashboard, forms, chart-table fallback, dialogs) so a11y wins don't quietly regress. Extends the charter's "the proof must be executable" principle from math to accessibility; process/technical, not user-visible. |
