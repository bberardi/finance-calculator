...
### Phase 19 — Data Integrity & Dashboard Clarity — _target v2.8_

Surfaced by a September 2026 codebase/issue review. Unlike Phases 16–18 (which
consolidated proposed _features_), this phase is driven by what the **open issue
list** actually shows: every one of the 24 open issues is a bug, and a cluster
of them share a single structural root cause the roadmap has never tracked —
the import boundary and the edit forms validate independently. The other two
items add the one _relationship_ view the dashboard still lacks and keep the
honesty panel honest as its assumptions become user-adjustable. Each passes the
§5 non-goal filters (client-side, data stays on device, not a budgeting app);
the rationale column says why each earns a slot.

**Data integrity (technical / robustness)**

| #    | Work item                                                                  | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ---- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.1 | **Single shared validation schema (import ↔ form parity by construction)** | The JSON import boundary (`data-helpers.ts`) and the per-type edit-form validators (`validation-helpers.ts`) enforce the same rules in **two hand-synced places** — the form code is littered with "mirrors the JSON import boundary" comments — and they keep drifting apart. A recurring class of open bugs is exactly this drift: import accepts a value the edit form then rejects, so the round-tripped position is **uneditable** (a `null` optional housing field #201, a `Principal`/`CurrentAmount` of 0 #194, a negative `GrowthRate` #184), or the form accepts what import rejects (`+Infinity` #183). A related import-boundary robustness gap throws a raw `TypeError` instead of the friendly validation error on malformed raw JSON the edit forms never receive — a truthy non-string `Id` (#178), a `null` element in the loans array (#202), a `null` top-level payload (#175) — the import boundary lacking an object/type guard the forms have no occasion to run. Define **one schema/parser per model that both boundaries call** (with the import side owning the raw-shape guards the forms don't need), so a rule cannot exist on only one side. Retires the whole uneditable-round-trip class by construction rather than one bug at a time. No schema-version bump; upholds D7 (pure, framework-free) and routes through the D8 migration ladder unchanged. Math-free, so the Charter's reference tests don't apply, but the shared parser gets its own boundary test matrix. |

**Dashboard insight (UX / visualization)**

| #    | Work item                      | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.2 | **Key financial ratios strip** | The dashboard is four **scalar** cards (`summary-helpers.ts` → total debt / assets / net worth / monthly commitments); the planned 14.3 adds a **composition** view (dollar _amounts_ by category). Neither shows the **relationships** that answer "where should my next dollar go?" most directly: a debt-to-asset ratio, a liquid-vs-illiquid split (via a documented `AssetType` proxy — cash as liquid, property and custom as illiquid — with the caveat that CDs and retirement/brokerage investments can't be told apart from account type alone; an explicit user-set liquidity flag, a schema change, is the fallback only if that proxy proves too coarse), a cash-runway figure (months of monthly commitments covered by cash), and — most on-point — the balance-weighted average loan rate **beside** the balance-weighted average expected investment return, which is the debt-vs-invest decision in a single line. Each ratio specifies an explicit empty state (an em dash, never `NaN` or a misleading `0%`) for a zero denominator — no debt, no assets, or no monthly commitments. Purely derived from the positions; no engine change, and no schema change under the type-based proxy. Distinct from 14.3 (amounts) and the optimizer (per-plan scoring). |

**Honesty & trust (extends Phase 9 / Phase 15)**

| #    | Work item                          | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ---- | ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.3 | **Live, linked assumptions panel** | The assumptions panel states its assumptions as **fixed copy** — `assumptions-panel.tsx` interpolates the exported default inflation constant (`DEFAULT_INFLATION_PCT`, currently 3) into "discount at N%/yr" and hardcodes "an after-tax view is still planned." Several of those assumptions are becoming user-adjustable (15.1 after-tax toggle, 17.5 user-set inflation rate), at which point fixed copy will contradict the live setting — the exact in-app-copy-vs-plan mismatch the roadmap already cites to justify 15.1 and 16.2. Make each panel line read the user's **actual current setting** (inflation %, after-tax on/off, Monte Carlo on/off) and link it to the control that changes it, so the honesty framing stays honest and becomes a navigation aid. No engine change, but not purely cosmetic: reading the Monte Carlo state means lifting `monteCarlo` out of `ForecastChart` (today a private `useState`, while the panel renders as a prop-less sibling in `Body.tsx`) into shared state, so the item carries that modest state-wiring. Best sequenced after 15.1/17.5 land so it reflects real controls. |
