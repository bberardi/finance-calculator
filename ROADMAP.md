...
### Phase 19 — Privacy, Data Resilience & Validation Integrity — _target v2.8_

Surfaced by an August 2026 follow-up UX/codebase review. The consolidated
Phases 16–18 sharpened the _forecast_ and the _what-if_ surface; this phase
turns to the parts of the "no backend, data stays on your device" promise the
earlier reviews left thin — the one durable copy of a user's data is a
**plaintext** file, the app keeps **no history** to fall back on, two open tabs
can silently **clobber** each other, and the import boundary and the edit forms
disagree about what a valid position is, stranding imported data in an
**uneditable** state. Each item passes the §5 non-goal filters (client-side,
data stays on device, not a budgeting app); the rationale column says why each
earns a slot. None touches the forecast/amortization math.

**Privacy & data safety**

| #    | Work item                             | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---- | ------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.1 | **Passphrase-encrypted export**       | The headline privacy claim is "your numbers never leave your device," yet the one off-device copy — the JSON backup (`export-download.ts` writes a plaintext `application/json` blob) — is fully readable by anything that touches the file (a Downloads folder, a synced cloud drive, an email attachment). Offer an **optional** passphrase-encrypted export (Web Crypto `SubtleCrypto`, e.g. AES-GCM with PBKDF2 key-derivation) that import can decrypt with the passphrase, so a backup can be as private as the app itself. Plaintext JSON stays the default for portability; encryption is backend-free and key-free (the user holds the passphrase). Closes the gap between the in-app privacy copy and the artifact the app actually produces. |
| 19.2 | **Rolling on-device backup history**  | The only recovery paths today are the single-step soft-undo (delete/import merges; edits after 12.4) and a manual export — a bad import, a mass-delete confirmed past the snackbar, or a corrupt hydrate has **no** fallback. Keep the last _N_ committed states in a bounded `localStorage` ring (opt-in with persistence) and offer a "restore a previous version" picker. Distinct from 12.4 (one edit), 18.3 (plan-vs-reality snapshots), and 18.4 (a reminder to export): this is an automatic, local, multi-step safety net. Bounded and inputs-only, so it stays within the D4 storage model.                     |
| 19.3 | **Multi-tab state synchronization**   | With persistence on and the app open in two tabs, there is no `storage`-event listener anywhere (`use-persistence.ts` writes but never subscribes), so each tab holds a stale copy and whichever saves last silently overwrites the other's work — a real data-loss path for anyone who keeps a pinned tab. Subscribe to the `storage` event and reconcile (or prompt) when another tab commits a change. Small, backend-free, and closes a silent-overwrite gap that the planned unsaved-changes guard (13.1) and backup reminder (18.4) don't cover.                                                                    |

**Validation integrity (UX / robustness)**

| #    | Work item                                      | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 19.4 | **Unified validation schema (import ⇄ forms)** | The JSON-import boundary (`data-helpers.ts` / `validation-helpers.ts`) and the per-entity edit forms (`add-edit-loan.tsx`, `add-edit-investment.tsx`, `add-edit-asset.tsx`) apply **different** constraints, so import accepts values the edit form then rejects — a paid-off loan (`Principal`/`CurrentAmount` 0, #194), a negative-return investment (#184), an explicit-`null` optional housing field (#201) — leaving an **uneditable round-trip**, and some malformed inputs throw a raw `TypeError` instead of the friendly validation error (#202, #178, #175, #183). Extract one shared field-constraint schema both sides consume, so "importable" and "editable" are the same set by construction and every rejection is a friendly message. Retires a whole open-bug class rather than patching it case by case; lands under the Charter (§4) as helper-layer logic with tests. |

**Credibility (extends Phase 15.1)**

| #    | Work item                                | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ---- | ---------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.5 | **Per-account tax treatment (extends 15.1)** | The planned after-tax view (15.1) applies **one** flat effective rate to **all** investment gains — but a Roth/401(k) balance is taxed nothing-like a taxable brokerage account, so a single global haircut over-taxes tax-advantaged accounts and understates them. Add a per-investment tax-treatment tag (**taxable / tax-deferred / tax-free**) so the 15.1 haircut only bites the accounts it should. Still pure post-processing on the nominal forecast (no engine change) and still not a tax _return_ — it stays inside the §5 "no tax advice" line exactly as 15.1 does, while making the after-tax number honest across account types. Ships only alongside or after 15.1. |
