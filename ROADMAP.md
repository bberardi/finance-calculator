...
- **Process rules**: a formula change and its reference tests ship in the same PR; every math bug found — by review, by a user, by chance — gets a failing regression test committed before the fix; UI components never re-implement math (helpers are the single source of truth, which the "show the math" feature in Phase 7 will also rely on).
...
_Unblocks everything else — and fixes the baseline experience so every later phase builds on a shell worth looking at. Items 0.1–0.5 are architecture/platform; 0.6–0.10 are the UX overhaul; 0.11 implements the Math Correctness Charter (§4)._
...
Phase 0  Foundations + UX overhaul                    v0.7.0
         (engine, context, CI, dep modernization,
          ...
          math verification suite — gate for Phase 2)
