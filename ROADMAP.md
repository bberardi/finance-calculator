...
| 8.5 | **Strategy comparison view** ✅ _shipped v1.10.0_   | Side-by-side table in the optimizer: net worth at +5y/+10y/+30y, debt-free date, and the final investments-vs-debt split across the baseline and every preset, with the best long-run strategy flagged. Reuses the milestone engine (now scenario-aware) so figures match the dashboard.                                        |
