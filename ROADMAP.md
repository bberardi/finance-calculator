...
### Phase 19 — Trustworthy Inputs & Dashboard Honesty — _target v2.8_

Surfaced by an August 2026 codebase/issue review. The §4 Math Correctness
Charter gives the pure calculation layer executable proof, and it works — the
open-issue list holds almost no _formula_ bugs. Instead, every one of the
currently-open issues lives at the boundaries the charter does **not** cover:
the JSON-import validator disagreeing with the edit-form validator, dashboard
scalars disagreeing with the forecast that renders right below them, and
converters silently dropping fields. These have been fixed one issue at a time
(#72, #99) but keep recurring because no single item makes the **input and
display boundaries** provably consistent the way §4 makes the math provably
correct. This phase closes that gap. Each item passes the §5 non-goal filters
(client-side, data stays on device, not a budgeting app); the rationale column
says why each earns a slot.

**Data integrity (correctness — the input-boundary analogue of §4)**

| #    | Work item                                      | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| ---- | ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.1 | **Import ↔ form validation parity charter**    | The JSON-import boundary (`data-helpers.ts`, e.g. `n >= 0`) and the edit-form validators (`validation-helpers.ts`, e.g. `Principal > 0`) enforce **different** rules in both directions: a file can import a position the form then refuses to save (a paid-off loan with `CurrentAmount: 0`, #194; a negative-return investment #184), and the form accepts values (+Infinity #183) the import rejects. Five open issues (#194, #184, #183, #178, #175) are instances of this one divergence, patched case-by-case yet recurring. Make one shared validation module the single source both boundaries call, guarded by a round-trip property test — anything the form can save must import and vice-versa — so the class can't reappear. This is the input-side counterpart to §4's "proof, not review confidence." |
| 19.2 | **Dashboard scalars derive from the forecast** | "Monthly commitments" counts a fully-paid-off loan (#179) and a future-dated investment's contribution (#173) that the forecast itself applies **$0** for, so the headline number contradicts the chart directly below it. Derive the dashboard scalars (`net-worth-summary.tsx`) from the same forecast engine the chart already uses, so the summary and the projection can never disagree. Purely derived, no engine or schema change.                                                                                                                                                                                                                                                                                                                                                                            |
| 19.3 | **Provably lossless converters & clones**      | Editing or cloning an investment silently drops its `ResearchLinks` (#172), and Monarch import collapses same-named accounts into one asset (#167) — both routine actions that lose user-entered data with no warning. Make every converter/clone path lossless under a property test (convert-and-back preserves all fields; distinct inputs stay distinct), protecting the 9.4 research-links context already shipped and the Monarch flow (README) from silent data loss.                                                                                                                                                                                                                                                                                                                                         |

**Quality / technical**

| #    | Work item                            | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ---- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.4 | **Input-boundary integration tests** | §4 covers `src/helpers/**` at 100%, but the import → edit → export round-trip, the form-validation boundary, and the converters — where the entire current open-issue list actually lives — have only "pragmatic" coverage. Add focused integration tests over that round-trip and the validation boundary, extending the executable-proof ethos to the surfaces generating the bugs. Complements 19.1/19.3 by locking their invariants in as regression tests. |

**Dashboard insight (UX — extends Phase 14)**

| #    | Work item                           | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.5 | **Derived financial-health ratios** | The dashboard reports four raw scalars but no _context_ for them. Add honest, derived ratios from data the app **already** holds — loan-to-value (already computed for PMI drop-off), cash-liquidity runway (cash accounts ÷ monthly commitments), and debt-to-income (monthly commitments ÷ the salary already captured for the 401(k) match). Strictly derived from existing inputs, so it adds **no** expense/transaction tracking and stays inside the §5 line; it answers "am I financially healthy?" next to 14.3's "where is my money?" composition view. The most debatable item here — include only if the DTI/liquidity framing reads as position-context, not budgeting. |
