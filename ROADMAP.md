...
## 10. Proposed Additions (2026-06-17 review)

A fresh triage pass, same convention as §9 (and PRs #60, #67, #84): small, concrete, code-grounded items the current roadmap doesn't yet capture, each filed by category with a one-line rationale and, where applicable, a target phase. Purely additive — no existing section is rewritten. Items already planned or already proposed in §9 are deliberately omitted to avoid restating the plan.

### 10.1 — Roadmap hygiene / tracking

- **Refresh §9.1's tracking premise — it has gone stale, in the good direction.** §9.1 worries that the #68–#83 bug cluster is open and unreflected in §8; that cluster has since been **closed** (#87), and the two consistency bugs filed afterward (#88, #91) were **fixed in merged #96**. The live tracker now holds a _different_ open set — **#93** (month-end investment date divergence), **#94** (import accepts loans the edit form rejects), **#95** (three dormant defensive-hardening gaps) — and §8, which exists precisely for cross-cutting correctness items, still names none of them. _Rationale: the roadmap's "what's tracked where" should follow the live tracker; fold §9.1 down to "resolved" and migrate the genuinely-open correctness bugs (#93/#94) into §8 so the backlog stays the single home for them — the same "don't overstate status" standard §4 holds the math to._

### 10.2 — Correctness / systemic invariants

- **Make import↔form-validation parity a _tested_ invariant (Charter §4 extension), Phase 6 / §8.** Three bugs share one exact shape — #72, #85, and the still-open **#94** — where `importFromJson` admits an entity that `validateLoan`/`validateInvestment` then **rejects**, trapping a cleanly-imported entity as uneditable. Each has been patched one field at a time (interest rate, dates, and now `MonthlyPayment`). _Rationale: a property/consistency test asserting that every entity accepted by the import boundary is also acceptable to the form validator (and that the two agree on which fields are required) would catch this whole class structurally instead of reactively — exactly the cross-implementation-consistency discipline §4 layer 2 already applies to the two forecast engines, pointed at the import/form boundary._
- **Promote §8.2 (dayjs date-math migration) — it now backs a live, user-visible bug (#93).** §8.2 currently frames the `Date.setMonth` month-end rollover (Jan 31 + 1 month → Mar 3) as a latent Phase-6 cleanup pinned only by a tripwire test. Issue **#93** shows it is no longer latent: any investment with a day-29–31 start date gets a permanent one-period disagreement between `getInvestmentPeriods`/`forecastInvestment` and the canonical `generateInvestmentGrowth`, so the PIT view's period count contradicts its own value and the forecast chart diverges from the Growth Schedule popout. _Rationale: #93's own suggested fix is option (a) — migrate the schedule engine to dayjs month arithmetic, which already clamps month-ends — i.e. it **is** §8.2; re-tag §8.2 as the fix for #93 and lift it above generic Phase 6 cleanup, since it now produces contradictory numbers in shipped surfaces rather than a theoretical edge case._

### 10.3 — UX

- **Make a blocked Save self-explanatory and recoverable for _derivable_ fields, Phase 6.** When an imported loan with a missing/`0` `MonthlyPayment` is opened in the Edit dialog, Save is disabled (`validateLoan` requires `MonthlyPayment > 0`) while the recompute that would fill it is deliberately suppressed for existing loans (#56) — so the user faces a greyed-out button with no obvious way forward (the underlying #94). The form already has a `formErrors` channel "so the user can always see why Save is disabled," and the forecast engine already derives an amortizing payment for an unset value (`getEffectiveMonthlyPayment`, from #96). _Rationale: surfacing the blocking reason **with** a one-click "Recompute payment" affordance turns a dead-end into a guided fix, and is the UX complement to whichever boundary-parity fix #94 lands — small, reuses logic that already exists, and generalizes to any future derivable required field._
