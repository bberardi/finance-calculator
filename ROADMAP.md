...
### Phase 19 — Asset Realism, Privacy & Access — _target v2.8_

Surfaced by a September 2026 UX/codebase review. Four earlier reviews built out
the loan/what-if surface (Phases 16–18); this one closes the gaps that fall on
the **other** side — the asset engine, the privacy story the app leads with, a
power-user data need, and two accessibility/quality gaps the prior a11y passes
left open. Each item passes the §5 non-goal filters (client-side, data stays on
device, not a budgeting app); the rationale column says why each earns a slot.

**Forecast credibility (extends Phase 16 — Cashflow & Rate Realism)**

| #    | Work item                                         | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ---- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 19.1 | **Asset-side scheduled rate changes & glidepath** | The forecast holds every asset's `GrowthRate` fixed for the whole horizon — one field carrying cash APY, investment return, **and** property appreciation (`asset-model.ts`). Phase 16.2 adds dated rate changes for **loans only**, leaving the asset side asymmetric: a CD/HYSA whose promo APY steps down, a portfolio that de-risks toward retirement (a return "glidepath"), and a cooling appreciation regime all project at a single rate the app itself flags as unrealistic (`assumptions-panel.tsx`: "Average, not guaranteed"). Add the same optional ordered list of dated rate changes 16.2 gives loans to assets. Deterministic, backend-free, on the shared monthly axis. **Touches the forecast math, so the Math Correctness Charter (§4) applies** — cited reference tests, upheld invariants, 100% `src/helpers/**` coverage. |

**Data safety & privacy (extends Phase 13 / Phase 18)**

| #    | Work item                                              | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---- | ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.2 | **Optional passphrase encryption at rest & on export** | The product leads with "your numbers never leave your device," yet the on-device copy (the `pathwise:data` `localStorage` entry) and the JSON export are **plaintext** — readable by anyone with access to the browser profile or the exported file, the real exposure on a shared or family computer. Offer an opt-in passphrase that encrypts the export (and optionally `localStorage`) entirely client-side via Web Crypto (AES-GCM + PBKDF2) — no backend, no key escrow, a clear "a forgotten passphrase is unrecoverable" warning. Hardens the founding privacy promise; pairs with the 18.4 backup reminder and 18.5 selective import. |

**Power-user UX (extends Phase 13)**

| #    | Work item                    | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.3 | **Named on-device profiles** | Only one dataset persists (the whole picture lives under the single `pathwise:data` `localStorage` key), so modeling a second household, or a "current" vs. "planned life" world, forces manual export/import juggling. Named, switchable profiles kept on-device let a user hold several complete pictures at once and flip between them, reusing the existing serialization and the D8 validation/migration boundary. Client-side, on-device, not a budgeting feature; distinct from the 11.1 shareable link (a snapshot to hand off) and the 18.3 snapshots (points on one timeline). |

**Accessibility & quality (extends Phase 12 / Phase 15)**

| #    | Work item                                | Rationale / acceptance                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 19.4 | **Text-scaling & high-contrast support** | Phase 12 covered motion, keyboard, and live regions and 15.3 covered chart color, but nothing tracks reflow at 200% zoom / large OS font size (WCAG 1.4.4 / 1.4.10) or the OS high-contrast preference (`prefers-contrast` / forced-colors). Audit for fixed-`px` layouts that clip or overflow when text is scaled and honor the high-contrast signal in the MUI theme. Cheap, no engine impact, finishes the accessibility story the earlier passes started.                                                                                                      |
| 19.5 | **Visual-regression guard on key views** | The math core is exhaustively unit-tested, but the thing users actually _read_ — the forecast chart, the dashboard, the tables/cards in light and dark mode — has only a single smoke E2E (`e2e/smoke.spec.ts`) and no snapshot guard, so a rendering regression (clipped legend, broken dark-mode contrast, a chart axis gone wrong) can ship silently. Add a small Playwright screenshot baseline over a handful of seeded, deterministic views to catch visual regressions in CI. Test-only; extends the "trust the numbers" ethos to trusting what's on screen. |
