...
- 🏠 **Whole net worth** — cash accounts (HYSA/CD/checking), property (with home-equity from a linked mortgage), and custom assets/liabilities feed the net-worth roll-up alongside loans and investments. `CustomLiability` entries subtract from net worth; all others add. A property row also opens an **enhancement ROI** calculator — enter an improvement's cost and the value it adds, and see how much it recoups, its immediate net-worth effect, and (as the added value appreciates with the home) when it breaks even.
- 📎 **Research & context** — attach your own reference links (with optional notes) to an investment or property row — company filings, fund pages, local-market and area reports. Stored on-device and carried through export/import, but **never fetched**, so the app stays offline and key-free; only safe `http(s)` links are accepted.
