...
- 🦋 **Monarch import** — populate assets/liabilities from Monarch Money's "account balance history" CSV exports (select one or more — Monarch exports one file per account). Each account's latest balance maps to a custom asset or, when negative, a custom liability; re-importing updates the same entries in place. Runs entirely on-device through the same review-and-undo flow as JSON import.
